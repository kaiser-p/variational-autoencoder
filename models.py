...
class SimpleConvolutionalAE(nn.Module):
    def __init__(self, img_dim: int, bottleneck_dim: int, verbose: bool = False):
        ...
        self.encoder_layer_conv_1 = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=3, stride=1, padding=1)  # Output: [B, C=3, D, D]
    ...
    def forward(self, x):
        ...
        x = self.sigmoid(self.decoder_layer_tconv_1(x))
        return x
